...
fn resolve_version_interpolation(source: &str) -> String {
    if let Some(version) = parse_version(source) {
        let mut resolved = source.replace("#{version}", &version);
        let parts = version.split(',').collect::<Vec<_>>();
        for (placeholder, value) in [
            ("#{version.csv.first}", parts.first().copied()),
            ("#{version.csv.second}", parts.get(1).copied()),
            ("#{version.csv.third}", parts.get(2).copied()),
        ] {
