...
impl BuildExecutor {
    ...
    pub async fn execute(
        ...
        apply_isolated_dep_env(&mut env, installed_deps);
        env.insert(
...
pub struct DepInfo {
    ...
    pub isolated_prefix: Option<String>,
}

fn apply_isolated_dep_env(
    ...
    for dep in installed_deps.values() {
        let Some(prefix) = dep.isolated_prefix.as_ref() else {
            continue;
        };
        let prefix = Path::new(prefix);
        bin_paths.push(prefix.join("bin").display().to_string());
        pkg_config_paths.push(prefix.join("lib/pkgconfig").display().to_string());
        cflags.push(format!("-I{}", prefix.join("include").display()));
        ldflags.push(format!("-L{}", prefix.join("lib").display()));
    }

    prepend_path_list(env, "PATH", bin_paths);
    prepend_path_list(env, "PKG_CONFIG_PATH", pkg_config_paths);
    prepend_words(env, "CFLAGS", cflags.clone());
    prepend_words(env, "CPPFLAGS", cflags);
    prepend_words(env, "LDFLAGS", ldflags);
}

fn prepend_path_list(env: &mut HashMap<String, String>, key: &str, paths: Vec<String>) {
    if paths.is_empty() {
        return;
    }
...
fn prepend_words(env: &mut HashMap<String, String>, key: &str, words: Vec<String>) {
    if words.is_empty() {
        return;
    }
    let existing = env.get(key).cloned().unwrap_or_default();
