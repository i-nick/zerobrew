...
fn test_curl_keg_only() {
    ...
    // Keg-only executables are exposed through the zerobrew bin directory,
    // while private support files remain isolated in the Cellar.
    assert!(
        t.bin_dir().join("curl").exists(),
        "curl should be linked into the zerobrew bin directory"
    );
...
fn test_list_installed_formulas() {
    ...
    let output = t.zb(&["list", "--all"]);
    assert_success(&output, "zb list --all");
    assert_stdout_contains(&output, "jq");
